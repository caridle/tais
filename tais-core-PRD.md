...
| 💬 **企业微信机器人** | 接收群聊消息、路由到 TAIS 技能、注入基因风格、返回回复 | Webhook |
...
| S2 | ✅ LLM 真实调用 | 统一 LLM 客户端支持 OpenAI/Anthropic/Ollama，已接入 parser/optimizer/gene |
| S3 | MCP stdio 传输 | 完整支持 stdio 子进程传输 |
| S4 | 习惯跨代遗传 | Agent 派生时继承父 Agent 的习惯权重 |
| S5 | 企业微信机器人 | 通过企业微信群机器人 webhook 接入，学生群内对话 AI |
...
gantt
    ...
    section Phase 2 增强
    LLM 真实调用                   :done, p2a, 2026-05-10, 1d
    PostgreSQL 持久化              :p2b, after p2a, 2d
    企业微信机器人                 :p2c, after p2a, 2d
    MCP stdio 传输                 :p2d, after p2c, 2d
    TAIS 7胶囊注册                 :p2e, after p2d, 2d
    习惯胶囊实现                   :p2f, after p2e, 3d
...
| 阶段 | 内容 | 状态 |
|------|------|------|
| **Phase 1 MVP** | Rust 骨架 + Orchestrator + MCP + Evolution + Gene + API + 测试 | ✅ 已完成 |
| **Phase 2 增强** | LLM 调用 ✅ + PostgreSQL + MCP stdio + WeChat 企业微信机器人 | 进行中 |
